import { ICreditReport, CreditBureau } from '../models/CreditReport';
import { AggregationMethod } from '../models/AggregatedCreditScore';

// Define weights for each bureau for weighted averaging
const DEFAULT_WEIGHTS = {
  [CreditBureau.EXPERIAN]: 0.35,
  [CreditBureau.EQUIFAX]: 0.35,
  [CreditBureau.TRANSUNION]: 0.3
};
 ...
 */
export const aggregateCreditScores = (
  ...
  components: Array<{ bureau: CreditBureau; score: number; weight: number; reportId: string }>;
  ...
  const missingBureaus = Object.values(CreditBureau).filter(
    bureau => !availableBureaus.includes(bureau)
  ) as CreditBureau[];

  // Filter out only available reports
  const validReports = reports.filter(report => report.isAvailable && report.data?.creditScore);
  ...
  switch (method) {
    ...
    case AggregationMethod.HIGHEST:
    ...
    case AggregationMethod.MEDIAN:
      ...
      if (sortedScores.length % 2 === 0) {
        ...
        aggregatedScore = Math.round(
          (sortedScores[middleIndex - 1].score + sortedScores[middleIndex].score) / 2
        );
      } else {
      ...
      if (sortedScores.length % 2 === 0) {
        const medianIndex1 = components.findIndex(
          c => c.bureau === sortedScores[middleIndex - 1].bureau
        );
        const medianIndex2 = components.findIndex(
          c => c.bureau === sortedScores[middleIndex].bureau
        );
        if (medianIndex1 !== -1) components[medianIndex1].weight = 0.5;
        if (medianIndex2 !== -1) components[medianIndex2].weight = 0.5;
      } else {
        const medianIndex = components.findIndex(
          c => c.bureau === sortedScores[middleIndex].bureau
        );
        if (medianIndex !== -1) components[medianIndex].weight = 1;
      }
      break;
